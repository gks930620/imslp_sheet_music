...
public class ComposerQueryService {
    ...
    /** §3-5 — 공개 곡 1개 이상, name_ko 가나다순(없으면 뒤로) → name_original 순. */
    public ComposerListDTO list() {
        List<Row> rows = publicRows();
    ...
    /** §3-6 — 공개 곡 수 많은 순(동률 name_ko 순). */
    public List<ComposerCardDTO> featured(int limit) {
        if (limit < 1) {
